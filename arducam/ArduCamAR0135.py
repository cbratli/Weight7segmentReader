...
global cfg,handle,running,Width,Heigth,save_flag,color_mode,save_raw, imageQueue
running = True
save_flag = False
save_raw = False
cfg = {}
...
def sigint_handler(signum, frame):
    ...
    if rtn_val == 0:
        print("device close success! In start camera after while")
    else:
...
def startCamera(config_file_name):
    global running, handle,ct,rt
    if camera_initFromFile(config_file_name):
        ...
        #rtn_val = ArducamSDK.Py_ArduCam_close(handle)
        #if rtn_val == 0:
        #    print("device close success! Inside start camera")
        #else:
        #    print("device close fail!")
...
def startMainCamera(conf):
    ...
    if not os.path.exists(config_file_name):
        ...
        showHelp()
    startCamera(config_file_name)

if __name__ == '__main__':
    startMainCamera(sys.argv[1])
